...
class EmailClient {
  ...
  /**
   * getUserCommand -
   * Prompts user for a number corresponding to a given command. Checks if
   * valid, then returns corresponding command type.
   *
   * @param input - Scanner that points to source of input (e.g. console)
  ...
  /**
   * fetchMail -
   * Requests the server for the mail of the current user. Prints each email out
   * to console
   *
  ...
  /**
   * composeNewMail -
   * Prompts user to input a to user field and a body text field. Delivers mail
   * to specified user
   *
